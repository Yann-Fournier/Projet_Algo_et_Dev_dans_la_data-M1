...
options = Options()
options.add_argument("--log-level=3")  # réduit les logs
options.add_argument("--disable-notifications")
options.add_argument("--disable-background-networking")
options.add_argument("--headless=new")  # mode headless moderne
options.add_argument("--disable-gpu")   # option utile sur Windows

# Créez une instance de navigateur Chrome
driver = webdriver.Chrome(options=options)
base_url = "https://www.amazon.fr"

fichier_json_url_Categories = os.path.join(current_dir, 'CSV/1_Categories.json')
fichier_json_Pages = os.path.join(current_dir, 'CSV/1_Pages.json')
...
print("Peut-on scraper la page d'accueil ? ", rp.can_fetch("*", base_url + "/")) # True
print("delay entre les requêtes : ", rp.crawl_delay("*")) # None
print()
...
try:
    driver.find_element(By.XPATH, '/html/body/div[1]/div[1]/form/div[2]/div/span[2]/span/span/input').click()
except:
...
if doit_verife_cookie:
    try:
        driver.find_element(By.CSS_SELECTOR, '#sp-cc-rejectall-link').click()
    except:
    ...
    for i in range(1, 2): # On ne scrap que 5 page par ce que ce n'est qu'un exemple de code
        ...
        try:
            ...
            for link in linksInPage:
                ...
                # Nom --------------------------------------------------------------------------------------------------
                try:
                    name = driver.find_element(By.CSS_SELECTOR, '#productTitle').text
                except:
                    name = ""
                if name == "":
                    try:
                ...
                # Description ------------------------------------------------------------------------------------------
                try:
                    desc = driver.find_element(By.CSS_SELECTOR,'#bookDescription_feature_div > div > div.a-expander-content.a-expander-partial-collapse-content > span').text
                except:
                    desc = ""
                if desc == "":
                    try:
                        desc = driver.find_element(By.XPATH,'///*[@id="bookDescription_feature_div"]/div/div[1]/span').text
                    except:
                        desc = ""
                if desc == "":
                    try:
                ...
                if pic == "":
                    try:
                        pic = driver.find_element(By.XPATH,'//*[@id="landingImage"]').get_attribute('src')
                    except:
                        pic = ""
                if pic == "":
                    try:
                        pic = driver.find_element(By.CSS_SELECTOR,'#landingImage').get_attribute('src')
                    except:
                        pic = ""
                photo.append(pic)
                ...
                try:
                    ...
                    pass
                try:
                    details = driver.find_elements(By.XPATH, '/html/body/div[1]/div[2]/div/div[26]/div/div[1]/ul/li')                                     
                    for det in details:
                        truc = det.find_element(By.XPATH, './span/span[1]').text
                        if truc.__contains__("ISBN-13"):
                            Isbn = det.find_element(By.XPATH, './span/span[2]').text
                ...
                if price == 0.0:
                    try:
                        priceSTR = driver.find_element(By.XPATH, '/html/body/div[2]/div/div[4]/div[1]/div[5]/div[4]/div[2]/div/div/div/div[2]/span/span/a/span[2]/span').text
                        priceSTR = priceSTR[:-2]
                ...
                if price == 0.0:
                    try:
                        priceSTR = driver.find_element(By.XPATH, '/html/body/div[2]/div/div[4]/div[1]/div[5]/div[4]/div[2]/div/div/div[2]/div[2]/span/span/a/span[2]/span').text
                        priceSTR = priceSTR[:-2]
                        priceSTR = priceSTR.replace(',', '.')
                        price = float(priceSTR)
                    except:
                ...
                if price == 0.0:
                    try:
                        priceSTR = driver.find_element(By.XPATH, '//*[@id="corePriceDisplay_desktop_feature_div"]/div[1]/span[2]/span[2]/span[1]').text
                        priceFloat = driver.find_element(By.XPATH, '//*[@id="corePriceDisplay_desktop_feature_div"]/div[1]/span[2]/span[2]/span[2]').text
                ...
                if price == 0.0:
                    try:
                        priceSTR = driver.find_element(By.CSS_SELECTOR, '#corePriceDisplay_desktop_feature_div > div.a-section.a-spacing-none.aok-align-center.aok-relative.apex-core-price-identifier > span.a-price.aok-align-center.reinventPricePriceToPayMargin.priceToPay.apex-pricetopay-value > span:nth-child(2) > span.a-price-whole').text
                        priceFloat = driver.find_element(By.CSS_SELECTOR, '#corePriceDisplay_desktop_feature_div > div.a-section.a-spacing-none.aok-align-center.aok-relative.apex-core-price-identifier > span.a-price.aok-align-center.reinventPricePriceToPayMargin.priceToPay.apex-pricetopay-value > span:nth-child(2) > span.a-price-fraction').text
                        price = priceSTR + "." + priceFloat
                        price = float(priceSTR)
                    except:
                        price = 0.0
            ...
            if len(nom) == len(description) == len(photo) == len(isbn) == len(editeur) == len(prix) == len(categorie):
                ...
                fileNameLivres = os.path.join(current_dir, 'CSV/Livres' + key + '.csv')
                if i == 1:
